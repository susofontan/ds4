...
- Through `ds4-server`, test OpenAI Chat data URIs, Responses `input_image`,
  ...
  base64, unsupported media, a prompt whose measured image tokens exceed the
  context, and bodies over 64 MiB must return 4xx without reading local files
  or making network requests. There is no fixed image count limit, so a cold
  replay with more than 16 images must still be accepted when it fits the
  context. Separately, run `tests/test_server_vision_growth.py` against a
  Qwen3.8 vision server to prove that a transcript replay with more than 16
  covered images keeps being accepted while each turn adds one image.
